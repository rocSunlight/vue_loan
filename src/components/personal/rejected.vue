<template>
  <jroll-infinite class="jp-control" id="jp-jroll3">
      ...
      <li class="mui-table-view-cell" v-for="item in info">
        <div class="title">
          <div class="titles">
            ...
            <span class="mui-badge green" v-text="status[item.status]" ></span>
          </div>
        </div>
        <ul class="list-data">
          <li>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created(){
      _this4 = this
      this.$http.get('http://'+this.$store.state.urlIp+'/api/user/task/status/3', {params: {token: this.token1}}).then(response => {
      ...
      'jroll-infinite': JRoll.VueInfinite({
        ...
        bottomed: function (complete) {
          ...
          if (me.page < ( (page === 1)? page = 1 : page = page - 1)) {
            ...
            _this4.$http.get('http://' + this.$store.state.urlIp + '/api/user/task/status/3', {
              params: {
                token: _this4.token1,
                p: _page4+1
              }
              ...
              if (typeof complete === 'function') {
                _this4.info = _this4.info.concat(jons)
                me.page = 0
                complete()
              }else{
                _this4.info = _this4.info.concat(jons)
