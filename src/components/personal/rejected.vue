...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created(){
      _this = this
      this.$http.get('api/user/task/status/3', {params: {token: this.token1.token}}).then(response => {
    ...
    components: {
      'jroll-infinite': JRoll.VueInfinite({
        ...
        bottomed: function () {
          ...
          if (_page2 < page) {
            _this.$http.get('api/user/task/status/3', {params: {token: _this.token1.token,p:_page2}}).then(response => {
