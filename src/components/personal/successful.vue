<template>
  <jroll-infinite class="jp-control" id="jp-jroll6">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created(){
      _this6 = this
      this.$http.get('http://'+this.$store.state.urlIp+'/api/user/task/status/1', {params: {token: this.token1}}).then(response => {
    ...
    components: {
      'jroll-infinite': JRoll.VueInfinite({
        ...
        bottomed: function (complete) {
          var me = this
          var page = _this6.total_page  //获取后台传来的数据页数
          ...
          if (me.page < ( (page === 1)? page = 1 : page = page - 1)) {
            ...
            _this6.$http.get('http://' + this.$store.state.urlIp + '/api/user/task/status/1', {
              params: {
                token: _this6.token1,
                p: _page6+1
              }
              ...
              if (typeof complete === 'function') {
                _this6.info = _this6.info.concat(jons)
                me.page = 0
                complete()
              }else{
                _this6.info = _this6.info.concat(jons)
