...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created(){
      _this = this
      this.$http.get('api/user/task/status/2', {params: {token: this.token1.token}}).then(response => {
    ...
    components: {
      'jroll-infinite': JRoll.VueInfinite({
        ...
        bottomed: function () {
          ...
          if (_page4 < page) {
            _this.$http.get('api/user/task/status/2', {params: {token: _this.token1.token,p:_page4}}).then(response => {
              $('.loadhide').addClass('loadshow') //显示加载
              setTimeout(function () {
                _page4++ // 请求成功将page加1，失败则不要改变page
